...
public:
	Vector4() = default;
	FORCEINLINE Vector4(const Vector2 &InV, bool IsPoint = true) : X(InV.X), Y(InV.Y), Z(0.0f) { W = IsPoint ? 1.0f : 0.0f; };
	FORCEINLINE Vector4(const Vector3 &InV, bool IsPoint = true) : X(InV.X), Y(InV.Y), Z(InV.Z) { W = IsPoint ? 1.0f : 0.0f; };
	...
	FORCEINLINE Vector4 operator-(const Vector4 &InV) const;
	FORCEINLINE Vector4 operator/=(float InSalce);
...
FORCEINLINE Vector4 Vector4::operator/=(float InSalce)
{
	X /= InSalce;
	Y /= InSalce;
	Z /= InSalce;
